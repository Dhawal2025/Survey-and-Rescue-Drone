...
class Edrone():
	...
	def altitude_set_pid(self,alt):
		self.Kp[2] = alt.Kp * 0.015 # This is just for an example. You can change the ratio/fraction value accordingly
		self.Ki[2] = alt.Ki * 0.008
		self.Kd[2] = alt.Kd * 0.2

	# ----------------------------Define callback function like altitide_set_pid to tune pitch, roll--------------
	def pitch_set_pid(self,pitch):
		self.Kp[1] = pitch.Kp * 0.015 # This is just for an example. You can change the ratio/fraction value accordingly
		self.Ki[1] = pitch.Ki * 0.008
		self.Kd[1] = pitch.Kd * 0.1

	def roll_set_pid(self,roll):
		self.Kp[0] = roll.Kp * 0.015 # This is just for an example. You can change the ratio/fraction value accordingly
	...
	#	8. Add error_sum
		...
		self.iterm[0]=(self.iterm[0]+self.error[0])*0.288
		self.iterm[1]=(self.iterm[1]+self.error[1])*0.28
		self.iterm[2]=(self.iterm[2]+self.error[2])*0.592
		self.out_roll = 7.845*self.error[0]+self.iterm[0]+343.2*(self.error[0]-self.prev_err[0])
		self.out_pitch = 7.2*self.error[1]+self.iterm[1]+345*(self.error[1]-self.prev_err[1])
		self.out_throttle = 10.3*self.error[2]+self.iterm[2]+323.2*(self.error[2]-self.prev_err[2])
	...
	# ------------------------------------------------------------------------------------------------------------------------
		...
		self.pitch_err.publish(self.error[1])
		self.roll_err.publish(self.error[0])		
