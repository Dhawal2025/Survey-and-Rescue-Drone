...
class Edrone():
	...
	def __init__(self):
		...
		self.setpoint = [0.2, 0, 23]
		self.waypoints = [[0.2, 0, 23],[3.5, -3.3, 23],[-4.4, -4.6, 20.7],[-6.0, 6.3, 18.3],[5.0, 5.3, 16.3]]
		self.level=0

<<<<<<< HEAD:scripts/position_hold_gazebo.py
		# Declaring a cmd of message type PlutoMsg and initializing values
=======
		#Declaring a cmd of message type edrone_msgs and initializing values
>>>>>>> 8f7335463174cdc576eb355dfa3268ad64a4f72a:scripts/position_hold_gazebo_boilerplate.py
		...
		self.error = [0, 0, 0]       # Array for storing the difference value between drone postition and current setpoint
		self.prev_err = [0, 0, 0]    # Array for storing previous error values
		self.iterm = [0, 0, 0]       # Array for storing integral values
		self.max_values = [2000, 2000, 2000]  # Array for storing maximum values of roll, pitch and throttle
		self.min_values = [1000, 1000, 1000]  # Array for storing minimum values of roll, pitch and throttle
	...
	def whycon_callback(self,msg):
		self.drone_position[0] = msg.poses[0].position.x   # accessing x co-ordinate from 'msg'

		# --------------------Set the remaining co-ordinates of the drone from msg--------------------------------------------		   
		self.drone_position[1]=msg.poses[0].position.y     # accessing y co-ordinate from 'msg'
		self.drone_position[2]=msg.poses[0].position.z     # accessing z co-ordinate from 'msg'
	...
	#	8. Add error_sum
		...
		# logic for detecting threshold if the drone is in required co-ordinates 
		if(self.level>=5):
			print "Path Completed"				
		elif((self.drone_position[0]>=self.waypoints[self.level][0]-0.2 and self.drone_position[0]<=self.waypoints[self.level][0]+0.2) and (self.drone_position[1]>=self.waypoints[self.level][1]-0.2 and self.drone_position[1]<=self.waypoints[self.level][1]+0.2) and (self.drone_position[2]>=self.waypoints[self.level][2]-0.5 and self.drone_position[2]<=self.waypoints[self.level][2]+0.5) ):
			self.level=self.level+1		
			if(self.level==5):
				print "Path Completed"
			else:				
				self.setpoint = self.waypoints[self.level]
				print "Reached Level " + str(self.level)
		...
		#limiting the minimum and maximum values for roll,pitch and throttle
	...
	# ------------------------------------------------------------------------------------------------------------------------
		...
		self.command_pub.publish(self.cmd) 		# publishing roll,pitch and throttle
		self.alt_err.publish(self.error[2])		# publishing error in altitude
		self.pitch_err.publish(self.error[0])   #publishing error in pitch
		self.roll_err.publish(self.error[1])    # publishing error in roll	
		self.neg_err.publish(-0.2)		
		self.pos_err.publish(0.2)
